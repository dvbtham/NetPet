...
using System.Linq;
using PetNet.Common;
using PetNet.Common.Services.Int64;
...
namespace PetNet.Service
{
    ...
    public class ProductService : IProductService
    {
        ...
        {
            if (!string.IsNullOrEmpty(keyword))
                return _productRepository.GetMulti(x => x.Name.Contains(keyword) || x.Description.Contains(keyword) && x.IsDeleted == false && x.Status, new[] { "ProductCategory" });
            else
                return _productRepository.GetMulti(x => x.IsDeleted == false && x.Status, new[] { "ProductCategory" });
        }
